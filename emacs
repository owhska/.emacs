...
;; Dired usa "motion state": hjkl/gg/G funcionam estilo vim,
;; mas as teclas de ação (d, x, m, u, R, C, ~, etc.) continuam
;; sendo as originais do dired, sem interferência do evil.
(evil-set-initial-state 'dired-mode 'motion)

;; Quando você entra em modo de edição (C-x C-q / wdired), aí sim
;; você quer normal/insert completos pra editar nomes de arquivo.
...
(with-eval-after-load 'dired
  (evil-define-key 'motion dired-mode-map
    ...
    "l"         'dired-find-file
    "h"         'dired-up-directory)) ;; opcional, mas é o par natural de "l" no vim

;;; === DIRED NO WINDOWS (ls-lisp) ===

(require 'ls-lisp)
(setq ls-lisp-use-insert-directory-program nil) ;; força ls-lisp, ignora ls.exe externo
(setq ls-lisp-dirs-first t)                     ;; pastas antes de arquivos
(setq ls-lisp-verbosity nil)                    ;; sem colunas de link-count/owner (ruído no Windows)
(setq dired-listing-switches "-al")             ;; ls-lisp não suporta bem "-h" (human readable)
(setq dired-use-ls-dired nil)                   ;; silencia o aviso "ls does not support --dired"

;; Recarrega diretório automaticamente se o arquivo mudar fora do Emacs
(setq dired-auto-revert-buffer t)

;; Evita prompt irritante ao abrir outro diretório na mesma janela do dired
